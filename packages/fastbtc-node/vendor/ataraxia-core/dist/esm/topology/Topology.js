...
import { PeerMessageType, encodeId, sameId } from 'ataraxia-transport';
...
export class Topology {
    ...
    constructor(parent, options) {
        ...
        this.routing = new Routing(this.debug.namespace, this.self, this.nodes, id => {
            const d = this.peers.get(id);
            return d ? d[0].peer : undefined; // TODO: Pick lowest latency
        }, this.availableEvent, this.unavailableEvent);
        this.messaging = new Messaging(this.debug.namespace, this.routing, this.dataEvent);
    }
    /**
     * Start this topology.
     *
    ...
    handleNodeSummaryMessage(peer, message) {
        if (this.debug.enabled) {
            this.debug('Incoming NodeSummary from', encodeId(peer.id), 'version=', message.ownVersion, ', nodes=', message.nodes.map(n => encodeId(n.id) + ' ' + n.version));
        }
        const idsToRequest = [];
    ...
    gossipLatencies() {
        ...
        for (const node of this.nodes.values()) {
            if (node === this.self || sameId(node.id, peer.id))
                continue;
