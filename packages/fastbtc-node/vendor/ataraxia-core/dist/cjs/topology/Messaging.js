 ...
 */
class Messaging {
    constructor(debugNamespace, routing, dataEvent) {
        this.debug = debug_1.default(debugNamespace + ':messaging');
    ...
    send(target, type, data) {
        ...
        if (!peer)
            return Promise.reject(new Error('Node not reachable'));
        ...
        if (this.debug.enabled) {
            this.debug('Sending message', messageId, 'to', ataraxia_transport_1.encodeId(target));
        }
        return new Promise((resolve, reject) => {
            ...
            peer.send(ataraxia_transport_1.PeerMessageType.Data, message)
                .catch(ex => {
    ...
    handleData(peer, data) {
        if (ataraxia_transport_1.sameId(data.target, this.routing.self.id)) {
            // This is the target
            const lastPathEntry = data.path[data.path.length - 1];
            if (this.debug.enabled) {
                this.debug('Received message, sending ACK', lastPathEntry.id, 'to', ataraxia_transport_1.encodeId(peer.id));
            }
            peer.send(ataraxia_transport_1.PeerMessageType.DataAck, { id: lastPathEntry.id })
            ...
            if (!targetPeer || containsNode(data.path, targetPeer.id)) {
                 ...
                 */
                if (this.debug.enabled) {
                    this.debug('Unable to forward message, sending REJECT', lastPathEntry.id, 'to', ataraxia_transport_1.encodeId(peer.id));
                }
                peer.send(ataraxia_transport_1.PeerMessageType.DataReject, { id: lastPathEntry.id })
            ...
            if (this.debug.enabled) {
                this.debug('Forwarding message, sending ACK', lastPathEntry.id, 'via', messageId, 'to', ataraxia_transport_1.encodeId(peer.id));
            }
            // Forward the message to the target peer
            targetPeer.send(ataraxia_transport_1.PeerMessageType.Data, message)
                .catch(err => {
                this.releaseId(messageId);
    ...
    handleAck(ack) {
        ...
        if (message.resolve) {
            ...
            if (!peer) {
                if (this.debug.enabled) {
                    this.debug('Unable to forward ACK', ack.id, 'to', ataraxia_transport_1.encodeId(previous.peer));
                }
                return;
            }
            if (this.debug.enabled) {
                this.debug('Forwarding ACK', ack.id, 'to', ataraxia_transport_1.encodeId(peer.id));
            }
            peer.send(ataraxia_transport_1.PeerMessageType.DataAck, { id: previous.id })
                .catch(err => this.debug('Caught error while sending DataAck to peer', err));
        }
    }
    /**
    ...
    handleReject(reject) {
        ...
        if (message.reject) {
            ...
            if (!peer) {
                if (this.debug.enabled) {
                    this.debug('Unable to forward REJECT', reject.id, 'to', ataraxia_transport_1.encodeId(previous.peer));
                }
                return;
            }
            if (this.debug.enabled) {
                this.debug('Forwarding REJECT', reject.id, 'to', ataraxia_transport_1.encodeId(peer.id));
            }
            peer.send(ataraxia_transport_1.PeerMessageType.DataReject, { id: previous.id })
                .catch(err => this.debug('Caught error while sending DataReject to peer', err));
        }
    }
}
 ...
 */
function containsNode(path, id) {
    for (const p of path) {
        if (ataraxia_transport_1.sameId(p.node, id))
            return true;
    }
    return false;
}
